...
import { Heart, Sparkles, ExternalLink, Play } from 'lucide-react';

export default function HasratHub() {
  const [hasratUrl] = useState<string>(() => {
    return localStorage.getItem('hasrat_linked_url_hasrat') || 'https://www.Asupanlendir.com';
  });

  const handleOpenLink = () => {
    try {
      window.open(hasratUrl, '_blank', 'noopener,noreferrer');
    } catch (err) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-neutral-800 pb-6 mb-8">
        <div>
          ...
          <p className="text-sm text-neutral-400 mt-1">
            Kumpulan video sensasi premium dan tontonan panas paling dicari malam ini.
          </p>
        </div>
        ...
        >
          ...
          <span>Buka Portal Utama ({hasratUrl.replace(/^https?:\/\/(www\.)?/, '')})</span>
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-3 max-w-xl">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-pink-900/40 border border-pink-500/30 text-pink-400 text-xs font-bold font-mono tracking-wider uppercase">
              ...
              <span>REKOMENDASI CANDY POPULER</span>
            </div>
            
            <h3 className="text-2xl md:text-3xl font-extrabold text-white leading-tight font-sans">
              Akses Semua Video Terpopuler Tanpa Batas Tanpa Hambatan
            </h3>
            
            <p className="text-neutral-400 text-sm leading-relaxed">
              Dapatkan pembaruan langsung video viral dewasa terlengkap setiap hari. Server cepat, kualitas menakjubkan, dan bebas biaya selamanya.
            </p>
          </div>
